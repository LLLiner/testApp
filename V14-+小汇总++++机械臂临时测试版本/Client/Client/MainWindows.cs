...
namespace Client
{
    ...
    public partial class MainWindows : Form
    {
        ...
        static bool armIsLink = false;
        static Thread thrGrab;
        static Thread thrWave;
        static Thread thrStartStack;
        static Thread thrStopStack;

        // 用于侧无线时延
        static Thread pingThr;
        ...
        public int maxUpDelay = 20;
        public int minUpDelay = 2;
        ...
        */
        ...
        private void setMData()
        {
            ...
            try
            {
                ...
                chart.AxisY.Interval = (maxDelay - minDelay) / 10;
            ...
            try
            {
                ...
                chartA2.AxisY.Interval = (maxUpDelay - minUpDelay) / 10;
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        //获取offset数据
        private DataTable getOffsetDB()
        {
            MySqlConnection myConnect = new MySqlConnection(constructorString);
            myConnect.Open();
            MySqlCommand myCmd = new MySqlCommand("SELECT * from `offset` WHERE date_sub(now(),interval 2 minute) < time;", myConnect);

            DataTable dt = new DataTable();
            DataSet ds = new DataSet();
            MySqlDataAdapter adapter = new MySqlDataAdapter(myCmd);
            adapter.Fill(ds, "offsetTable");
            dt = ds.Tables["offsetTable"];
            ...
            if(debug == 1)
            {
                Console.WriteLine("共获取到" + dt.Rows.Count + "条数据");

                foreach (DataRow dr in dt.Rows)
                {
                    Console.WriteLine("id: " + dr["id"] + " \t offsetValue：" + dr["offsetValue"] + "\t  time：" + dr["time"]);
                }
            }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < 8; i++)
            {
                ...
                if (this.chart2.Series[i].Points.Count == 0)
                {
                    ...
                    {
                        Console.WriteLine("");
                        Console.WriteLine("Series[{0}].Points = null\n", i);
                    }
            ...
            DataTable dtArm = getDBDataByOwner(2);
            ...
            chart1.Series[2].Points.DataBind(dtArm.AsEnumerable(), "time", "delay", "");
            ...
            */
            ...
            chart1.Series[0].Name =  " 灯";
            chart1.Series[1].Name = " 马达";
            chart1.Series[2].Name = " 机械臂";



            //offset数据展示
            DataTable dtOffset = getOffsetDB();
            this.offsetChart.Series[0].Points.Clear();
            
            this.offsetChart.Series[0].Points.DataBind(dtOffset.AsEnumerable(), "time", "offsetValue", "");

            this.offsetChart.ChartAreas[0].AxisX.Maximum = DateTime.Now.ToOADate();
            this.offsetChart.ChartAreas[0].AxisX.Minimum = DateTime.Now.AddSeconds(-60).ToOADate();
        ...
        private void MainWindows_Load(object sender, EventArgs e)
        {
            ...
            chart1.Series[2].MarkerBorderColor = Color.Purple;
            ...
            //offset图展示
            this.offsetChart.ChartAreas[0].AxisY.Minimum = 0;
            this.offsetChart.ChartAreas[0].AxisY.Maximum = 50;
            this.offsetChart.ChartAreas[0].AxisY.Interval = 5;

            offsetChart.Series[0].MarkerBorderColor = Color.MediumVioletRed;
            offsetChart.Series[0].MarkerBorderWidth = 2; //chart1.;// Xaxis 
            offsetChart.Series[0].MarkerColor = Color.White;//AxisColor
            offsetChart.Series[0].MarkerSize = 5;
            offsetChart.Series[0].MarkerStyle = MarkerStyle.Circle;

            offsetChart.Series[0].XValueType = ChartValueType.DateTime;
        ...
        //设置主时钟
        private void chooseMaster(object sender, EventArgs e)
        {
        ...
        {
            this.Hide();
            if (new setForm().ShowDialog() == DialogResult.OK)
            {
            }
            this.Show();
        }
        ...
        private void grabMessage()
        {
            if (armIsLink == false)
            {
                sshClient = new SshClient("192.168.1.114", 22, "ubuntu", "hiwonder");
                sshClient.Connect();
                armIsLink = true;
            }
            var cmd = sshClient.CreateCommand("cd ArmPi_PC_Software/; python3 TSN.py");
            var res = cmd.Execute();
            if(res == null)
            {
                Console.WriteLine("机械臂抓取出错啦！");
            }
            thrGrab.Abort();
        }
        ...
        {
            thrGrab = new Thread(grabMessage); //new一个接收数据的线程
            thrGrab.Start();
        }
        ...
        private void waveMessage()
        {
            ...
            var cmd = sshClient.CreateCommand("cd ArmPi_PC_Software/; python3 test.py");
            var res = cmd.Execute();

            if (res == null)
            {
                Console.WriteLine("机械臂波动出错啦！");
            }
            thrWave.Abort();
        ...
        {
            thrWave = new Thread(waveMessage); //new一个接收数据的线程
            thrWave.Start();

        }

        private void startStackMessage()
        {
            if (armIsLink == false)
            {
                sshClient = new SshClient("192.168.1.114", 22, "ubuntu", "hiwonder");//这里因为使用了ssh所以端口一定要使用22
                sshClient.Connect();
                armIsLink = true;
            }

            //String str = "rosservice call /object_pallezting/enter \"{}\";";
            //String str = "echo \"$HOME\""; //这个带双引号的命令可正常输出，说明不是双引号和转义字符\的锅
            //String str = "echo $PATH";
            //String str1 = "PATH=$PATH:/home/ubuntu/.local/bin:/opt/ros/melodic/bin:/usr/bin/python;echo $PATH;rosservice call /object_pallezting/enter \"{}\";";
            //String str1 = "cd /opt/ros/melodic/bin;ls -l rosservice";
            //String str1 = "PYTHONPATH=$PYTHONPATH:/home/ubuntu/armpi_fpv/devel/lib/python2.7/dist-packages:/opt/ros/melodic/lib/python2.7/dist-packages;echo $PYTHONPATH;PATH=$PATH:/home/ubuntu/.local/bin:/opt/ros/melodic/bin:/usr/bin/python;echo $PATH;rosservice call /object_pallezting/enter \"{}\";";

            /*String str1 = "PATH=$PATH:/home/ubuntu/.local/bin:/opt/ros/melodic/bin:/usr/bin/python;" +
                "export PYTHONPATH=/home/ubuntu/armpi_fpv/devel/lib/python2.7/dist-packages:/opt/ros/melodic/lib/python2.7/dist-packages;echo $PYTHONPATH;" +
                "rosservice call /object_pallezting/enter \"{}\";" +
                "rosservice call /object_pallezting/set_running \"data: true\";";
                */

            //由于采用ssh登录，有的环境变量和本地登录不一样，所以需要前面两句设置环境变量
            String str1 = "export PYTHONPATH=/home/ubuntu/armpi_fpv/devel/lib/python2.7/dist-packages:/opt/ros/melodic/lib/python2.7/dist-packages;echo $PYTHONPATH;" +
                "PATH=$PATH:/home/ubuntu/.local/bin:/opt/ros/melodic/bin:/usr/bin/python;" +
                "rosservice call /object_pallezting/enter \"{}\";" +
                "rosservice call /object_pallezting/set_running \"data: true\";";

            var cmd1 = sshClient.CreateCommand(str1);
            Console.WriteLine(cmd1.CommandText);
            var res1 = cmd1.Execute();

            if (cmd1.ExitStatus == 0)
            {
                Console.WriteLine("-----------cmd1执行成功啦！----------");
                Console.WriteLine(cmd1.Result);//执行结果
            }
            else
            {
                Console.WriteLine("--------------cmd1执行失败-------------");
                Console.WriteLine(cmd1.Error);//错误信息
            }
        }

        private void button5_Click(object sender, EventArgs e)
        {
            thrStartStack = new Thread(startStackMessage); 
            thrStartStack.Start();
        }

        private void stopStackMessage()
        {
            if (armIsLink == false)
            {
                sshClient = new SshClient("192.168.1.114", 22, "ubuntu", "hiwonder");
            ...
            var cmd = sshClient.CreateCommand("PATH=$PATH:/home/ubuntu/.local/bin:/opt/ros/melodic/bin:/usr/bin/python;" +
                "export PYTHONPATH=/home/ubuntu/armpi_fpv/devel/lib/python2.7/dist-packages:/opt/ros/melodic/lib/python2.7/dist-packages;" +
        ...
        {
            thrStopStack = new Thread(stopStackMessage); 
            thrStopStack.Start();
        }
        ...
        private void pingDelay()
        {
            ...
            for (int i = 0; i < tryTime; i++)
            {
                ...
                if (reply.Status == IPStatus.Success)
                {
                    ...
                    countTime = countTime + reply.RoundtripTime;
                    if (reply.RoundtripTime > max)
                    {
                        max = reply.RoundtripTime;
                    }
                    else if (reply.RoundtripTime < min)
                    {
            ...
            float rttAva = countTime / tryTime;
            Console.WriteLine("最大RTT时间：" + max + "ms");
            Console.WriteLine("最小RTT时间：" + min + "ms");
            Console.WriteLine("平均RTT时间：" + rttAva + "ms");
            Console.WriteLine("平均单向时间：" + rttAva / 2.0 + "ms");
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            //pingThr = new Thread(pingDelay); //new一个接收数据的线程
            //pingThr.Start();

            this.Hide();
            if (new wireless().ShowDialog() == DialogResult.OK)
            {
            }
            this.Show();
